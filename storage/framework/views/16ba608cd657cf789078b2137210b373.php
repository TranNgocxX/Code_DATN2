...
<div class="max-w-lg mx-auto px-4 py-8">
    <!-- Nút quay lại -->
    <div class="mb-4">
        <a href="<?php echo e(route('bookings.index')); ?>" class="inline-flex items-center text-[#6B8F71] hover:text-[#557A5E] transition group">
    ...
    <div class="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden">
        ...
        <div class="text-center px-6 pt-6 pb-4 border-b border-slate-100">
            <span class="inline-block px-3 py-1 text-xs font-semibold rounded-full border <?php echo e($statusData['class']); ?>">
                <?php echo e($statusData['label']); ?>

            </span>
            <h2 class="text-2xl font-bold text-slate-800 mt-2">#<?php echo e($booking->id); ?></h2>
            <p class="text-sm text-slate-500">Đặt lúc <?php echo e($booking->created_at->format('H:i, d/m/Y')); ?></p>
        </div>
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between gap-4 bg-slate-50 p-4 rounded-xl">
                <div>
                    ...
                    <p class="font-semibold text-slate-700"><?php echo e($booking->service->name); ?></p>
                </div>
                <div class="sm:text-right">
                    <p class="text-xs text-slate-400 uppercase">Thời gian hẹn</p>
                    <p class="font-semibold text-[#557A5E]">
                        <?php echo e($booking->start_time->format('H:i d/m/Y')); ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="font-semibold text-slate-700"><?php echo e($booking->appointmentDetail->customer_name ?? $booking->user->name ?? 'N/A'); ?></p>
                    <p class="text-sm text-slate-500"><?php echo e($booking->appointmentDetail->phone ?? 'Không có SĐT'); ?></p>
                </div>
                <div>
                    <p class="text-xs text-slate-400 uppercase">Chuyên viên</p>
                    <p class="font-semibold text-slate-700"><?php echo e($booking->employee->name ?? 'Đang sắp xếp'); ?></p>
            ...
            <div class="flex flex-wrap gap-2">
                <span class="px-3 py-1 bg-slate-100 rounded-lg text-xs font-bold text-slate-600">
                    <?php echo e(strtoupper($booking->payment_method ?? 'cash')); ?>

                </span>
                <span class="px-3 py-1 rounded-lg text-xs font-bold <?php echo e($booking->payment_status == 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'); ?>">
                    <?php echo e($booking->payment_status == 'paid' ? 'Đã thanh toán' : 'Chưa thanh toán'); ?>

                </span>
            </div>

            <?php if($booking->appointmentDetail?->health_status): ?>
                <div>
                    <p class="text-xs text-slate-400 uppercase">Tình trạng sức khỏe</p>
                    <p class="text-slate-600 text-sm mt-1"><?php echo e($booking->appointmentDetail->health_status); ?></p>
                </div>
            <?php endif; ?>

            <?php if($booking->appointmentDetail?->notes): ?>
                <div>
                    <p class="text-xs text-slate-400 uppercase">Ghi chú</p>
                    <p class="italic text-slate-500 text-sm mt-1">
                            "<?php echo e($booking->appointmentDetail->notes); ?>"
                    </p>
                </div>
            <?php endif; ?>
            ...
            <div class="flex justify-between items-center border-t pt-4">
                ...
                <span class="text-xl font-bold text-red-800">
                    <?php echo e(number_format($booking->price ?? 0)); ?>đ
                </span>
            </div>
        </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/Code_DATN/resources/views/user/bookings/show.blade.php ENDPATH**/ ?>