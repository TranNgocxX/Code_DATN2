    ...
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/60 pb-5">
            ...
            <div class="flex items-center gap-4">
                <span class="text-xs text-slate-400 font-medium hidden lg:inline italic">Cập nhật: <?php echo e(now()->format('H:i - d/m/Y')); ?></span>
                <div class="inline-flex rounded-xl bg-slate-200/60 p-1 text-xs font-bold shadow-inner">
                    <?php $__currentLoopData = ['week' => '7 ngày', 'month' => '30 ngày', 'quarter' => '90 ngày']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="<?php echo e(request()->fullUrlWithQuery(['period' => $key])); ?>" 
                           class="px-4 py-2 rounded-lg transition-all duration-200 <?php echo e($currentPeriod == $key ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'); ?>">
                            <?php echo e($label); ?>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
            <?php $__currentLoopData = [
                ...
                <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden group">
                    <div class="absolute top-0 left-0 h-1 w-full <?php echo e($meta[2]); ?> opacity-70"></div>
                    <div class="text-slate-400 text-[10px] font-black uppercase tracking-wider"><?php echo e($meta[0]); ?></div>
                    <div class="text-2xl font-black <?php echo e($meta[1]); ?> mt-2 tracking-tight group-hover:scale-105 transition-transform origin-left">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm flex flex-col overflow-hidden">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <h3 class="font-bold text-slate-800 text-sm uppercase tracking-wider flex items-center">
                    ...
                    Dòng thời gian: <span class="ml-2 text-sky-600 lowercase font-normal"><?php echo e(\Carbon\Carbon::parse(request('date', now()))->format('d/m/Y')); ?></span>
                </h3>
                
                <div class="relative min-w-[200px]">
                    <input type="date" id="timelineDatePicker" value="<?php echo e(request('date', now()->format('Y-m-d'))); ?>"
            ...
            <div class="overflow-x-auto py-4 flex gap-4 scrollbar-thin">
                <?php $__empty_1 = true; $__currentLoopData = $timelineData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $slot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="inline-block min-w-[180px] bg-slate-50/50 border border-slate-100 rounded-2xl p-4 align-top flex-shrink-0">
                        <div class="text-[11px] font-bold text-slate-400 border-b border-slate-200/60 pb-2 mb-3 flex justify-between items-center">
                            <span><?php echo e($slot['time']); ?></span>
                            <?php if(count($slot['appointments']) > 0): ?>
                        ...
                        <div class="space-y-3 max-h-[300px] overflow-y-auto scrollbar-none">
                            <?php $__empty_2 = true; $__currentLoopData = $slot['appointments']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $app): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_2 = false; ?>
                                <?php $style = $statusMap[$app->status] ?? $statusMap['pending']; ?>
                                <div class="p-3 rounded-xl text-xs border bg-white shadow-sm hover:shadow-md transition-all duration-200 <?php echo e($style['class']); ?>">
                                    <div class="font-black truncate text-slate-800"><?php echo e($app->user->name ?? 'Khách lẻ'); ?></div>
                                    <div class="text-[10px] text-slate-500 truncate mt-1 font-medium italic"><?php echo e($app->service->name ?? 'N/A'); ?></div>
                                    <div class="mt-3 flex items-center justify-between text-[9px] font-black border-t border-slate-100 pt-2">
                                        <span class="text-slate-400">⏱ <?php echo e(\Carbon\Carbon::parse($app->start_time)->format('H:i')); ?></span>
                                        <span class="uppercase tracking-widest"><?php echo e($style['label']); ?></span>
                                    </div>
                                </div>
            ...
            <div class="divide-y divide-slate-100 overflow-y-auto pr-1 flex-1 scrollbar-thin">
                <?php $__currentLoopData = $topServices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex items-center justify-between py-3.5 first:pt-0 last:pb-0 group">
                        <span class="text-sm font-semibold text-slate-600 group-hover:text-slate-900 transition-colors truncate mr-3"><?php echo e($service->name); ?></span>
                        <span class="text-xs font-black px-3 py-1 bg-pink-50 text-pink-600 border border-pink-100 rounded-full flex-shrink-0">
                            <?php echo e($service->appointments_count); ?> lượt
                        </span>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <a href="<?php echo e(route('admin.appointments.index')); ?>" class="px-4 py-2 bg-slate-50 hover:bg-slate-100 border border-slate-200/60 rounded-xl text-xs font-bold text-slate-700 transition-all shadow-sm flex items-center gap-1.5 w-fit">
                    Xem toàn bộ 
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/70 border-b border-slate-100">
                            <?php $__currentLoopData = ['Khách hàng', 'Thông tin dịch vụ', 'Thời gian hẹn', 'Trạng thái']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $th): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <th class="px-6 py-3.5 text-[10px] font-black text-slate-400 uppercase tracking-widest"><?php echo e($th); ?></th>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        <?php $__empty_1 = true; $__currentLoopData = $recentAppointments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <?php $style = $statusMap[$a->status] ?? $statusMap['pending']; ?>
                            <tr class="hover:bg-slate-50/40 transition-colors group">
                                <td class="px-6 py-4 font-bold text-slate-700 text-sm"><?php echo e($a->user->name ?? 'Khách lẻ'); ?></td>
                                <td class="px-6 py-4">
                                    <div class="font-semibold text-slate-700 text-sm"><?php echo e($a->service->name ?? '-'); ?></div>
                                    <div class="text-[11px] font-medium text-slate-400 mt-0.5"><?php echo e(number_format($a->service->price ?? 0)); ?>đ</div>
                                </td>
                                <td class="px-6 py-4 text-sm font-medium text-slate-500">
                                    <?php echo e(($a->start_time)->format('H:i • d/m/Y')); ?>

                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border <?php echo e($style['class']); ?>">
...
<script>
    ...
    // Biểu đồ trạng thái
    const statusData = <?php echo json_encode($chartData, 15, 512) ?>;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/Code_DATN2/resources/views/admin/dashboard/index.blade.php ENDPATH**/ ?>